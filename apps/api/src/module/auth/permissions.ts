 ...
 * - viewer 는 전부 read-only (단, pastoral 은 민감 → 제외).
...
const ALL_READS: Permission[] = ['member:read', 'finance:read', 'attendance:read', 'calendar:read', 'gallery:read'];
...
const ROLE_PERMISSIONS: Record<MembershipRole, Permission[]> = {
  [MembershipRole.OWNER]: [...ALL_READS, 'pastoral:read', ...SACRED_WRITES, 'finance:write', 'team:manage'],
  [MembershipRole.ADMIN]: [...ALL_READS, 'pastoral:read', ...SACRED_WRITES, 'finance:write', 'team:manage'],
  [MembershipRole.STAFF]: [...ALL_READS, 'pastoral:read', ...SACRED_WRITES],
