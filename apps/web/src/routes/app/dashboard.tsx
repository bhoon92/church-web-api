...
const STATS: Stat[] = [
  { label: '이번 주 출석', value: '—', delta: '', icon: CalendarCheck },
  { label: '이번 달 헌금', value: '—', delta: '', icon: Wallet },
  { label: '새가족', value: '—', delta: '', icon: UserPlus },
  { label: '예산 집행률', value: '—', delta: '', icon: TrendingUp },
];

type Schedule = { time: string; title: string; layer: string; hue: number };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        <SectionCard title="오늘의 일정" hint={`${UPCOMING.length}건`} action="달력 열기" className="lg:col-span-3">
          {UPCOMING.length === 0 ? (
