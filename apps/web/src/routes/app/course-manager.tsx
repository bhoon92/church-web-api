...
import { TRAINING_FORMAT_LABEL, createCourse, listCourses, updateCourse, type TrainingCourse, type TrainingFormat } from '@/api/training';
...
function CourseForm({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
  ...
  const createMut = useMutation({
    mutationFn: () => createCourse({ name: name.trim(), format, defaultSessionCount: Number(sessionCount) || 1 }),
    onSuccess: onDone,
