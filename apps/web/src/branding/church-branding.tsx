  ...
  const style = useMemo<CSSProperties>(
    () =>
      ({
        ...
        '--color-church-accent-foreground':
          branding?.accentForeground ?? DEFAULT_ACCENT_FG,
      }) as CSSProperties,
    [branding],
  )
...
export function useChurchBranding() {
  ...
  if (!ctx)
    throw new Error(
      'useChurchBranding must be used inside <ChurchBrandingProvider> (and inside an authenticated route)',
    )
  return ctx
}
